...
## 测试方法
&emsp;打开若干个命令行窗口，master一个，~~slave一个，分别在窗口调用`python replication-master.py`和`python replication-slave-threading.py`

## 主节点代码含义（replication-master.py）

&emsp;`results = Handler.run(host, port, data)`将建立对端口的监听，并对输入数据分片，交由工作节点计算，当所有计算任务完成时，此方法收集计算结果并返回

## 工作节点代码含义（replication-slave-threading.py）
